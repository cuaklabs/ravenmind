// ---------------------------------------------------------------------------
// V3 Memory Extraction Prompt
// Inspired from https://github.com/mem0ai/mem0/blob/a488e19044e4de9322a4e82be8d49ee67a322151/mem0/configs/prompts.py
// ---------------------------------------------------------------------------
export const DEFAULT_CUSTOM_INSTRUCTIONS: string = `Your Task: Extract durable, actionable facts from conversations between a user and an AI assistant. Only store information that would be useful to an agent in a FUTURE session, days or weeks later.
...
export const MEMORY_EXTRACTION_PROMPT: string = `
# ROLE
...
The current conversation turn(s) provided as a JSON array. Each element has a "role" field ("user" or "assistant") and a "content" field (a string for text-only messages, or an array of content parts for multi-modal messages).
...
When extracting a new memory, check if it relates to any Existing Memory. Add related Existing Memory IDs to "linkedMemoryIds". Link when:
...
Do NOT link memories that merely share a vague theme. Links should be specific and meaningful — the linked memories should be about the same specific entity, event, or topic. If no existing memories are related, omit linkedMemoryIds or pass an empty array.
...
Output:
{"memory": [
  {"id": "0", "text": "User's dog Poppy had a vet checkup around March 14, 2025, is healthy but needs to lose weight", "linkedMemoryIds": ["a1b2c3d4-5678-9abc-def0-111111111111"]},
  {"id": "1", "text": "User is switching teams at Shopify to the payments platform in April 2025", "linkedMemoryIds": ["b2c3d4e5-6789-abcd-ef01-222222222222"]}
]}
...
Output:
{"memory": [
  {"id": "0", "text": "John and his dog Max went on a camping trip in the summer of 2023 where they hiked, swam, and found it a peaceful experience", "linkedMemoryIds": ["a1b2c3d4-0000-0000-0000-111111111111"]},
...
{
  "memory": [
    {"id": "0", "text": "First extracted memory", "attributedTo": "user", "linkedMemoryIds": ["id-of-related-existing-memory"]},
    {"id": "1", "text": "Second extracted memory", "attributedTo": "assistant"}
  ]
}
...
- **attributedTo** (string, required): Who this memory is about. Use "user" for facts stated by or about the user (preferences, plans, personal facts). Use "assistant" for information provided by the assistant (recommendations, confirmations, plans created, information researched).
- **linkedMemoryIds** (array of strings, optional): IDs of Existing Memories that this new memory relates to. Use the exact IDs from the Existing Memories list. Omit or pass [] if no existing memories are related.
